...
export const addWeatherToUsers = async (users) => {
    return Promise.all(users.map(async (user) => {
        try {
            const coordinates = await getCoordinates(user.city, user.state, user.country);
            if (!coordinates) {
...
export const refreshUsersWeather = async (users) => {
    return Promise.all(users.map(async (user) => {
        try {
            const coordinates = await retry(() => getCoordinates(user.city, user.state, user.country), 3, 1000);
            if (!coordinates) {
                return { ...user, weather: null };
            }
